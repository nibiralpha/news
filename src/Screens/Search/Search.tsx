...
function Search() {
  ...
  const data: NewsListResponse = useSelector(
    (state: any) => state.searchSlice.data,
  );
  ...
  const fetchMore = async () => {
    ...
    dispatch(
      getTopNewsSuccess({
        ...data,
        results: [...data.results, ...newNews.results],
      }),
    );
  };
    ...
    <div className="App">
      <div className={`mb-30 mt-50 flex filter-area`}>
        <div>
          ...
          {!loading && data.results?.length == 0 && (
            <div className="mt-20 size-18 ">No serach results found</div>
          )}
        </div>
        <div>
          <SelectInput
      ...
      <div id="scrollableDiv">
        <InfiniteScroll
          ...
          loader={""}
          endMessage={
            <p style={{ textAlign: "center" }}>No more results found</p>
          }
        >
