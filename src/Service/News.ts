...
const getNews = async (filters: object): Promise<NewsListResponse> => {
  try {
    const response: AxiosResponse = await axios.get<NewsResponse>(
      `${API_END_POINT}/search?${generateQS(filters)}`,
    );
...
const getNewsById = async (
  ...
  try {
    const response: AxiosResponse = await axios.get<SingleNewsResponse>(
      `${API_END_POINT}/${id}?${generateQS(filters)}`,
    );
    return response.data.response;
  } catch (error: any) {
    throw error;
